...
function flattenProviderCategories(providers, categories) {
  ...
  const flattened = filteredProviders.map(provider => {
    ...
    for (const cat of categoryList) {
      record[cat] = providerCats.includes(cat) ? '✓' : ''
    }
...
function getMetroAreaData(parsed, metroArea) {
  const members = removeMetroAreaColumn(parsed['dump-member'].filter(m => m['Metro Area'] === metroArea))
  const membersWithDates = members.map(m => ({
    ...m,
    'Birthday': formatDateToISO(m['Birthday']),
    'Join Date': formatDateToISO(m['Join Date'])
  }))

  const requests = removeMetroAreaColumn(parsed['dump-service-requested'].filter(r => r['Metro Area'] === metroArea))
  const requestsWithDates = requests.map(r => ({
    ...r,
    'Created Date/Time': formatDateToISO(r['Created Date/Time']),
    'Start Date/Time': formatDateToISO(r['Start Date/Time']),
    'Finish Date/Time': formatDateToISO(r['Finish Date/Time'])
  }))

  const confirmed = removeMetroAreaColumn(parsed['dump-service-confirmed'].filter(c => c['Metro Area'] === metroArea))
  const confirmedWithDates = confirmed.map(c => ({
    ...c,
    'Created Date/Time': formatDateToISO(c['Created Date/Time']),
    'Start Date/Time': formatDateToISO(c['Start Date/Time']),
    'Finish Date/Time': formatDateToISO(c['Finish Date/Time'])
  }))

  const history = removeMetroAreaColumn(parsed['dump-service-history'].filter(h => h['Metro Area'] === metroArea && h['Service Name'] !== 'Member Added'))
  const historyWithDates = history.map(h => ({
    ...h,
    'Created Date/Time': formatDateToISO(h['Created Date/Time']),
    'Start Date/Time': formatDateToISO(h['Start Date/Time']),
    'Finish Date/Time': formatDateToISO(h['Finish Date/Time'])
  }))
  ...
  return {
    ...
    'Requests Confirmed': confirmedWithDates,
    'Requests History': historyWithDates
  }
}
...
function getProviderServiceCounts(historyRecords, confirmedRecords = []) {
  ...
  for (const record of historyRecords) {
    ...
    if (providerName.trim() && providerName !== 'Cancelled') {
      if (!providerCounts[providerName]) {
        providerCounts[providerName] = { completed: 0, confirmed: 0 }
      }
      providerCounts[providerName].completed += 1
    }
  }
  ...
  for (const record of confirmedRecords) {
    ...
    if (providerName.trim() && providerName !== 'Cancelled') {
      ...
      providerCounts[providerName].confirmed += 1
    }
  }
...
function countByField(fieldName, openRecords = [], confirmedRecords = [], historyRecords = []) {
  ...
  for (const record of openRecords) {
    ...
    if (name.trim()) {
      if (!counts[name]) {
        counts[name] = { open: 0, confirmed: 0, completed: 0, cancelled: 0 }
      }
      counts[name].open += 1
    }
  }
  ...
  for (const record of historyRecords) {
    ...
    if (name.trim()) {
      ...
      const cancellationReason = record['Cancellation Reason'] || ''
      if (cancellationReason && cancellationReason !== 'N/A' && cancellationReason.trim()) {
  ...
  return Object.entries(counts)
    .map(([name, count]) => ({ name, open: count.open, confirmed: count.confirmed, completed: count.completed, cancelled: count.cancelled }))
    .sort((a, b) => (b.open + b.confirmed + b.completed + b.cancelled) - (a.open + a.confirmed + a.completed + a.cancelled))
}
