...
function getMetroAreaData(parsed, metroArea) {
  ...
  const historyUnmatched = parsed['dump-service-history-unmatched'] ? removeMetroAreaColumn(parsed['dump-service-history-unmatched'].filter(h => h['Metro Area'] === metroArea && h['Service Name'] !== 'Member Added')) : []
  const historyUnmatchedWithDates = historyUnmatched.map(h => ({
    ...h,
    'Created Date/Time': formatDateToISO(h['Created Date/Time']),
    'Start Date/Time': formatDateToISO(h['Start Date/Time']),
    'Finish Date/Time': formatDateToISO(h['Finish Date/Time'])
  ...
  const allHistory = [...historyWithDates, ...historyUnmatchedWithDates].sort((a, b) => {
    const aNum = parseInt(a['Request Number']) || 0
    const bNum = parseInt(b['Request Number']) || 0
    return aNum - bNum
  })
...
function getProviderServiceCounts(historyRecords, confirmedRecords = []) {
  ...
  for (const record of historyRecords) {
    ...
    if (volunteerName.trim() && volunteerName !== 'Cancelled' && status === 'completed') {
