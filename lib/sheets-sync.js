...
async function syncTab(sheets, spreadsheetId, { tabName, records, providerNames = new Set(), metroArea = null, memberNames = new Set() }) {
  if (records.length === 0) {
  ...
  const rows = [headers, ...records.map(r => headers.map((h, colIdx) => {
    const value = r[h] || ''
    // Compute Is volunteer from cross-section name lookup — CSV field value is ignored
    if (tabName === 'Members' && colIdx === isVolunteerIdx) {
      return providerNames.has(r['Name'].trim()) ? '✓' : ''
    }
    // Compute IsMember from cross-section name lookup — CSV field value is ignored
...
async function syncMetroArea(sheets, spreadsheetId, { tabs, emailTimestamp, metroArea, parsed }) {
  ...
  const memberNames = new Set(members.map(m => m['Name'].trim()))

  for (const [tabName, records] of Object.entries(tabs)) {
    if (!tabName.startsWith('_')) {
      await syncTab(sheets, spreadsheetId, { tabName, records, providerNames, metroArea, memberNames })
    }
  }
