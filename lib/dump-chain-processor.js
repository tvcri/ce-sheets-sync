...
function getMetroAreaData(parsed, metroArea, asOf = new Date()) {
  ...
  const providerCounts = getProviderServiceCounts(historyForDisplay, currentConfirmedForDisplay)
  const memberCounts = getMemberRequestCounts(requestsWithDates, currentConfirmedForDisplay, historyForDisplay)
  const categoryCounts = getProviderCategoryCounts(categories)
  const serviceCounts = getServiceNameCounts(requestsWithDates, currentConfirmedForDisplay, historyForDisplay)
  const memberVolunteerCounts = getMemberVolunteerCounts(membersWithDates, cleanProviders)

  // Cross-section name sets
  const providerNames = new Set(providers.map(p => p['Name'].trim()))
  const memberNames = new Set(membersWithDates.map(m => m['Name'].trim()))
...
function getHubData(parsed, metroAreas) {
  // Global aggregations
  ...
  for (const metroArea of Object.keys(metroAreas)) {
  ...
  return { memberVolunteerCounts, categoryCounts, providerTotals, memberTotals, serviceCounts }
}
...
class DumpChainData {
  ...
  getHubData(metroAreas) {
    return getHubData(this.#parsed, metroAreas)
  }
}

export { DumpChainData }
