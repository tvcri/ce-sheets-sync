...
async function syncAll(conn, data, timestamp) {
  ...
  for (const raw of (parsed['dump-service-confirmed'] || [])) {
    requestRows.push(toServiceRequestRow(raw, raw['Status'] || 'Confirmed', raw['Volunteer']))
  }
  ...
  if (timestamp) {
    const isoTimestamp = new Date(timestamp).toISOString().slice(0, 19).replace('T', ' ')
    try {
      await conn.query('INSERT INTO ce_dump (ceDumpTime) VALUES (?)', [isoTimestamp])
    } catch (err) {
      console.error('Failed to insert ce_dump timestamp:', { isoTimestamp, error: err.message })
      throw err
    }
  }
}
