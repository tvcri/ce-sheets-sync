...
async function syncTab(sheets, spreadsheetId, tabName, records, providerNames = new Set(), metroArea = null) {
  if (records.length === 0) {
...
async function syncMetroArea(sheets, spreadsheetId, tabs, emailTimestamp, metroArea, parsed) {
  ...
  for (const [tabName, records] of Object.entries(tabs)) {
    if (!tabName.startsWith('_')) {
      await syncTab(sheets, spreadsheetId, tabName, records, providerNames, metroArea)
    }
  }
