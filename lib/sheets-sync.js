  ...
  try {
    ...
    if (serviceCounts.length > 0) {
      ...
      const serviceRows = serviceCounts.map(sc => [
        `${sc.name} (${sc.unmatched + sc.cancelled + sc.open + sc.confirmed + sc.completed})`,
        sc.unmatched,
        sc.cancelled,
        sc.open,
        sc.confirmed,
...
async function syncMetroArea(sheets, spreadsheetId, tabs, emailTimestamp, metroArea, parsed) {
  ...
  // Workaround: autoResizeDimensions assumes Arial 9, so we:
  // 1. Set font to Arial 10 (for auto-fit calibration)
  // 2. Apply autoResize
  // 3. Set font back to Arial 9
  ...
  try {
    ...
    if (tabSheets.length > 0) {
      // Step 1: Set all cells to Arial 10 (batch all tabs into one request)
      info(`Setting font to Arial 10 for auto-fit calibration`)
      const setArialTenRequests = tabSheets.map(tab => ({
        updateCells: {
          range: { sheetId: tab.sheetId, startRowIndex: 0, endRowIndex: 1000000 },
          rows: [{ values: [{ userEnteredFormat: { textFormat: { fontFamily: 'Arial', fontSize: 10 } } }] }],
      ...
      // Step 2: Apply autoResize (batch all tabs into one request)
      ...
      // Step 3: Set all cells back to Arial 9 (batch all tabs into one request)
      info(`Reverting font to Arial 9`)
      const setArialNineRequests = tabSheets.map(tab => ({
        updateCells: {
          range: { sheetId: tab.sheetId, startRowIndex: 0, endRowIndex: 1000000 },
          rows: [{ values: [{ userEnteredFormat: { textFormat: { fontFamily: 'Arial', fontSize: 9 } } }] }],
          fields: 'userEnteredFormat(textFormat(fontFamily,fontSize))',
          allCells: true
        }
      }))
      ...
      info(`Column auto-fit complete with font workaround`, { tabCount: tabSheets.length })
    }
  } catch (err) {
...
async function syncHub(sheets, spreadsheetId, parsed, emailTimestamp) {
  ...
  for (const metroArea of Object.keys(productionMetroAreas).filter(m => m !== 'TVC')) {
