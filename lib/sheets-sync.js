...
async function updateMetadata(sheets, spreadsheetId, metadata) {
  ...
  try {
    ...
    // Add member request counts to Metadata tab (row 40, columns H-L)
    if (memberCounts.length > 0) {
      const headers = ['Member', 'Cancelled', 'Open', 'Confirmed', 'Completed']
      const rows = memberCounts.map(mc => [mc.name, mc.cancelled, mc.open, mc.confirmed, mc.completed])
      requests.push(buildCellUpdateRequest(metadataSheetId, 7, headers, rows))
    }

    // Add provider category counts to Metadata tab (row 40, columns M-N)
    if (categoryCounts.length > 0) {
      const headers = ['Category', 'Count']
      const rows = categoryCounts.map(cc => [`${cc.name} (${cc.count})`, cc.count])
      requests.push(buildCellUpdateRequest(metadataSheetId, 12, headers, rows))
    }

    // Add service name counts to Metadata tab (row 40, columns O-S)
    if (serviceCounts.length > 0) {
      const headers = ['Service Name', 'Cancelled', 'Open', 'Confirmed', 'Completed']
      const rows = serviceCounts.map(sc => [`${sc.name} (${sc.open + sc.confirmed + sc.completed + sc.cancelled})`, sc.cancelled, sc.open, sc.confirmed, sc.completed])
      requests.push(buildCellUpdateRequest(metadataSheetId, 14, headers, rows))
    }
